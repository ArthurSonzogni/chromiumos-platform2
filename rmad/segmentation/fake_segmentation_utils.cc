...
constexpr char kLookedUpFeatureLevelKey[] = "looked_up_feature_level";
// Output JSON keys.
...
FakeSegmentationUtils::FakeSegmentationUtils(
    ...
    : working_dir_path_(working_dir_path),
      ...
      feature_level_(0),
      looked_up_feature_level_(std::nullopt) {
  ...
  if (input_dict->Initialized()) {
    ...
    if (int looked_up_feature_level; input_dict->GetValue(
            kLookedUpFeatureLevelKey, &looked_up_feature_level)) {
      looked_up_feature_level_ = looked_up_feature_level;
    }
  }
}
