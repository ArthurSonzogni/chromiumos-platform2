...
def _ExtentsToBspatchArg(extents, block_size, base_name, data_length=-1):
    ...
    for ex, ex_name in common.ExtentIter(extents, base_name):
        if not data_length:
            raise PayloadError(
                "%s: more extents than total data length" % ex_name
            )
    ...
    if data_length:
        raise PayloadError(
            "%s: extents not covering full data length" % base_name
        )
...
class PayloadApplier(object):
    ...
    def _ApplyReplaceOperation(
        ...
        data_start = 0
        for ex, ex_name in common.ExtentIter(
            op.dst_extents, "%s.dst_extents" % op_name
        ):
            ...
            if (start_block + num_blocks) * block_size > part_size:
                raise PayloadError(
                    ...
                    % (ex_name, common.FormatExtent(ex, block_size), part_size)
                )
    ...
    def _ApplySourceCopyOperation(
        self, op, op_name, old_part_file, new_part_file
    ):
    ...
    def _ApplyDiffOperation(
        self, op, op_name, patch_data, old_part_file, new_part_file
    ):
        ...
        if hasattr(new_part_file, "fileno") and (
            ...
            in_extents_arg, _, _ = _ExtentsToBspatchArg(
                ...
                if op.src_length
                else self._BytesInExtents(op.src_extents, "%s.src_extents"),
            )
            out_extents_arg, pad_off, pad_len = _ExtentsToBspatchArg(
                ...
                if op.dst_length
                else self._BytesInExtents(op.dst_extents, "%s.dst_extents"),
            )
    ...
    def _ApplyOperations(
        self, operations, base_name, old_part_file, new_part_file, part_size
    ):
        ...
        for op, op_name in common.OperationIter(operations, base_name):
            ...
            if op.type in (
                ...
                self._ApplySourceCopyOperation(
                    op, op_name, old_part_file, new_part_file
                )
            elif op.type in (
                common.OpType.SOURCE_BSDIFF,
                ...
                self._ApplyDiffOperation(
                    op, op_name, data, old_part_file, new_part_file
                )
            else:
                raise PayloadError(
                    "%s: unknown operation type (%d)" % (op_name, op.type)
                )

    def _ApplyToPartition(
        self,
        operations,
        part_name,
        base_name,
        ...
        with open(new_part_file_name, new_part_file_mode) as new_part_file:
            old_part_file = (
                open(old_part_file_name, "r+b") if old_part_file_name else None
            )
            try:
    ...
    def Run(self, new_parts, old_parts=None):
        ...
        if part_names - old_part_names:
            if self.payload.IsDelta():
                raise PayloadError(
                    "trying to apply a delta update without src "
                    "partition(s) %s" % ", ".join(part_names - old_part_names)
                )
        elif old_part_names == part_names:
            if self.payload.IsFull():
                raise PayloadError(
                    "trying to apply a full update onto src partitions"
                )
        else:
        ...
        for name, operations in install_operations:
            ...
            self._ApplyToPartition(
                operations,
                name,
                "%s_install_operations" % name,
                new_parts[name],
                new_part_info[name],
                old_parts.get(name, None),
                old_part_info[name],
            )
