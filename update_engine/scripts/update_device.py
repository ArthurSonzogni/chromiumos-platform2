...
class AndroidOTAPackage(object):
    ...
    def __init__(self, otafilename, secondary_payload=False):
        ...
        property_entry = (
            self.SECONDARY_OTA_PAYLOAD_PROPERTIES_TXT
            if secondary_payload
            else self.OTA_PAYLOAD_PROPERTIES_TXT
        )
...
class UpdateHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # pylint: disable=invalid-name
        ...
        start_range, end_range = self._parse_range(
            self.headers.get("range"), serving_size
        )
        logging.info(
            "Serving request for %s from %s [%d, %d) length: %d",
            self.path,
            self.serving_payload,
            serving_start + start_range,
            serving_start + end_range,
...
def main():
    ...
    if args.file:
        ...
        if not args.no_push:
            ...
            cmds.append(
                ["shell", "su", "0", "mv", data_local_tmp_file, device_ota_file]
            )
            cmds.append(
                [
                    "shell",
                    ...
                    "0",
                    "chcon",
                    "u:object_r:ota_package_file:s0",
                    device_ota_file,
                ]
            )
        cmds.append(
    ...
    try:
        ...
        if use_omaha:
            update_cmd = OmahaUpdateCommand(
                "http://127.0.0.1:%d/update" % DEVICE_PORT
            )
        else:
