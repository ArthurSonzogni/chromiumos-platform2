...
void LegacyDHCPCDProxyFactory::CleanUpDhcpcd(const std::string& interface,
  ...
  if (iter != pids_need_to_stop_.end()) {
    ...
    // SIGALRM, dhcpcd will try to release the lease first, then exit, but for
    // SIGTERM, dhcpcd will not send the RELEASE message, which can result in
    // a race condition if dhcpcd is currently releasing a lease.
