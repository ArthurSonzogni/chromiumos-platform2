...
MockMobileOperatorInfo::MockMobileOperatorInfo(EventDispatcher* dispatcher,
  ...
  ON_CALL(*this, mccmnc()).WillByDefault(ReturnRef(mccmnc_));
  ...
  ON_CALL(*this, serving_mcc_alpha2())
      .WillByDefault(ReturnRef(empty_serving_mcc_alpha2_));
...
void MockMobileOperatorInfo::SetMccMnc(std::string mccmnc) {
  mccmnc_ = mccmnc;
}
