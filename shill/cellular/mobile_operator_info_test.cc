...
TEST_F(MobileOperatorInfoMainTest, tethering_disallowed) {
  EXPECT_CALL(*home_, tethering_disallowed()).WillOnce(Return(true));
  EXPECT_TRUE(operator_info_->tethering_disallowed());
  EXPECT_CALL(*home_, tethering_disallowed()).WillOnce(Return(false));
