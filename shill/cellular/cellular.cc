...
void Cellular::ValidateApnTryList(std::deque<Stringmap>& apn_try_list) {
  ...
  apn_try_list.erase(std::remove_if(apn_try_list.begin(), apn_try_list.end(),
                                    [](const auto& item) {
                                      return !item.contains(kApnProperty);
                                    }),
                     apn_try_list.end());
}
...
bool Cellular::CompareApns(const Stringmap& apn1, const Stringmap& apn2) const {
  ...
  for (auto const& pair : apn1) {
    ...
    if (!apn2.contains(pair.first) || pair.second != apn2.at(pair.first)) {
      return false;
    }
...
bool Cellular::IsRequiredByCarrierApn(const Stringmap& apn) const {
  ...
  // set to true.
  return apn.contains(kApnSourceProperty) &&
         apn.at(kApnSourceProperty) == kApnSourceMoDb &&
         apn.contains(kApnIsRequiredByCarrierSpecProperty) &&
...
std::deque<Stringmap> Cellular::BuildApnTryList(
  ...
  if (!modb_required_apn_exists && service_) {
    ...
    for (auto custom_apn : custom_apns_info) {
      ...
      if (!apn_try_list.back().contains(kApnSourceProperty)) {
  ...
  for (auto apn : apn_list_) {
    ...
    DCHECK(apn.contains(kApnSourceProperty));
  ...
  for (const auto& apn : apn_list_) {
    ...
    if (!custom_apn_list && custom_apn_info &&
        ...
        apn.contains(kApnSourceProperty)) {
    ...
    if (apn.contains(kApnSourceProperty) &&
        apn.at(kApnSourceProperty) == kApnSourceMoDb) {
      if (is_same_as_last_good_apn) {
...
std::string Cellular::BuildEntitlementCheckAnomalyDetectorPrefix() {
  // This string format is used by the anomaly detector to identify these
  // events. Do not modify it without updating the anomaly detector.
  return std::format("Entitlement check failed: mccmnc={}:",
...
void Cellular::EntitlementCheck(EntitlementCheckResultCallback callback,
  ...
  if (!entitlement_check_callback_.is_null()) {
    LOG(ERROR) << BuildEntitlementCheckAnomalyDetectorPrefix()
