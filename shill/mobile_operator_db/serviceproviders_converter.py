...
class ServiceProvidersConverter(object):
    ...
    def Convert(self):
        ...
        self._xml_cdma_nodes = len(self._root.findall(".//cdma"))
        self._xml_gsm_nodes = len(self._root.findall(".//gsm"))
    ...
    def _RunStatsDiagnostics(self):
        ...
        logging.info(
            "Total number of XML CDMA nodes read [xml_cdma_nodes]: %d",
            self._xml_cdma_nodes,
        )
        logging.info(
            "Total number of XML GSM nodes read [xml_gsm_nodes]: %d",
            self._xml_gsm_nodes,
        )
        logging.info(
            "Total number of XML nodes read "
            "[xml_nodes = xml_cdma_nodes + xml_gsm_nodes]: %d",
            self._xml_cdma_nodes + self._xml_gsm_nodes,
        )

        logging.info(
            "Total number of protobuf MNOs dumped "
            "[protobuf_mnos_dumped]: %d",
            self._protobuf_mnos_dumped,
        )
        logging.info(
            "Total number of protobuf MVNOs dumped "
            "[protobuf_mvnos_dumped]: %d",
            self._protobuf_mvnos_dumped,
        )
        logging.info(
            "Total number of protobuf nodes dropped "
        ...
        logging.info(
            "Total number of protobuf CDMA MNOs " "[protobuf_cdma_mnos]: %d",
            self._protobuf_cdma_mnos,
        )
        logging.info(
            "Total number of protobuf GSM MNOs " "[protobuf_gsm_mnos]: %d",
            self._protobuf_gsm_mnos,
        )
        logging.info(
            "Total number of protobuf GSM MVNOs " "[protobuf_gsm_mvnos]: %d",
            self._protobuf_gsm_mvnos,
        )
        logging.info(
            "Total number of protobuf unique GSM MVNOs. "
            "[protobuf_gsm_unique_mvnos]: %d",
            self._protobuf_gsm_unique_mvnos,
        )
        logging.info("  (Some MVNOs may appear in multiple MNOs)")
        logging.info(
            "Total number of protobuf GSM MVNOs that are also MNOs. "
    ...
    def _EnrichNode(
        ...
        """
        ElementTree.SubElement(node, "country", {"code": country_code})
        provider_map = {}
        provider_map["type"] = provider_type
        if primary is not None:
            provider_map["primary"] = primary
        if roaming_required is not None:
            provider_map["roaming-required"] = roaming_required
        ElementTree.SubElement(node, "provider", provider_map)
        for name, lang in names:
            name_map = {}
            if lang is not None:
                name_map["xml:lang"] = lang
            name_node = ElementTree.SubElement(node, "name", name_map)
            name_node.text = name
    ...
    def _TransformXML(self):
        ...
        # while modifying it.
        nodes = {}
        for country_node in self._root.findall("country"):
            cur_country = country_node.get("code")
            for provider_node in country_node.findall("provider"):
                primary = provider_node.get("primary")
                roaming_required = provider_node.get("roaming-required")
                names = [
                    (name_node.text, name_node.get("xml:lang"))
                    for name_node in provider_node.findall("name")
                ]

                for gsm_node in provider_node.findall("gsm"):
                    nodes[gsm_node] = (
                        cur_country,
                        primary,
                        roaming_required,
    ...
    def _CheckAmbiguousMCCMNC(self, mcc, mnc):
        ...
        if mccmnc in self._mcc_mnc_by_mccmnc:
            ...
            if old_mcc != mcc or old_mnc != mnc:
                self._LogAndRaise(
                    "Ambiguous MCCMNC pairs detected: " "(%s, %s) vs. (%s, %s)",
                    old_mcc,
                    old_mnc,
                    mcc,
                    mnc,
        ...
        self._mcc_mnc_by_mccmnc["mccmnc"] = (mcc, mnc)

    def _GroupGSMNodesByMCCMNC(self):
        """Map all GSM nodes with same MCCMNC together."""
        for gsm_node in self._root.findall(".//gsm"):
            network_id_nodes = gsm_node.findall("network-id")
            if not network_id_nodes:
                logging.warning("Found a GSM node with no MCCMNC. ")
                self._gsm_nodes_no_mccmnc.add(gsm_node)
                continue

            for network_id_node in gsm_node.findall("network-id"):
                mcc = network_id_node.get("mcc")
                mnc = network_id_node.get("mnc")
    ...
    def _FindPrimaryNodes(self):
        ...
        self._primary_cdma_nodes = set(self._root.findall(".//cdma"))
        ...
        for mccmnc, nodes in self._gsm_nodes_by_mccmnc.iteritems():
            ...
            primary = None
            for node in nodes:
                provider_node = node.find("provider")
                if (
                    provider_node.get("primary")
                    and provider_node.get("primary") == "true"
                ):
                    if primary is not None:
                        self._LogAndRaise(
                            "Found two primary gsm nodes with MCCMNC["
                            "%s]: \n%s\n%s",
                            mccmnc,
                            self._PPrintXML(primary),
            ...
            if primary is None:
                logging.warning(
                    "Failed to find primary node with "
            ...
            self._protobuf_gsm_mvnos += len(self._mvnos[primary]) - len(
                existing_mvnos
            )
            unique_mvnos = unique_mvnos | mvnos

        self._primary_nodes = self._primary_cdma_nodes | self._primary_gsm_nodes
    ...
    def _SortOperators(self, node_list):
        ...
        node_list.sort(
            cmp=lambda x, y: cmp(
                sorted([z.text for z in x.findall("name")]),
                sorted([z.text for z in y.findall("name")]),
            )
        )
        ...
        node_list.sort(
            cmp=lambda x, y: cmp(
                x.find("country").get("code"), y.find("country").get("code")
            )
        )
    ...
    def _SpewProtobuf(self):
        ...
        self._SpewComment(
            "This file was generated from serviceproviders.xml, "
        ...
        for node in primaries:
            ...
            if node in self._mvnos:
                ...
                for mvno_node in mvnos:
                    ...
                    self._SpewMessageBegin("mvno")
                    self._SpewNameFilter(mvno_node)
                    self._SpewData(mvno_node)
                    self._SpewMessageEnd("mvno")
            self._SpewMessageEnd("mno")
            self._SpewLine()
    ...
    def _SpewNameFilter(self, node):
        ...
        if not name_list:
            self._LogAndRaise(
                "Did not find any name for MVNO. Can not create filter.\n" "%s",
                self._PPrintXML(node),
            )

        name = "|".join(name_list)
        self._SpewMessageBegin("mvno_filter")
        self._SpewEnum("type", "OPERATOR_NAME")
        self._SpewString("regex", name)
        self._SpewMessageEnd("mvno_filter")

    def _SpewData(self, node):
        self._SpewMessageBegin("data")

        self._SpewString("uuid", str(uuid.uuid4()))
        country_node = node.find("country")
        self._SpewString("country", country_node.get("code"))

        provider_node = node.find("provider")
        roaming_required = provider_node.get("roaming-required")
        if roaming_required is not None:
            self._SpewBool("requires_roaming", roaming_required)
        for name_node in sorted(node.findall("name")):
            self._SpewLocalizedNameNode(name_node)

        # GSM specific fields.
        for network_id_node in sorted(node.findall("network-id")):
            self._SpewString(
                "mccmnc",
                network_id_node.get("mcc") + network_id_node.get("mnc"),
            )

        for apn_node in sorted(node.findall("apn")):
            self._SpewMobileAPNNode(apn_node)

        # CDMA specific fields.
        for sid_node in sorted(node.findall("sid")):
            self._SpewString("sid", sid_node.get("value"))
        ...
        if (
            node.find("username") is not None
            or node.find("password") is not None
            or node.find("dns") is not None
        ):
            self._SpewMobileAPNNode(node)

        self._SpewMessageEnd("Data")

    def _SpewMobileAPNNode(self, apn_node):
        self._SpewMessageBegin("mobile_apn")
        apn = apn_node.get("value")
        # This may be None when converting a <cdma> node to MobileAPN node.
        if apn is None:
            apn = ""
        self._SpewString("apn", apn)
        for name_node in sorted(apn_node.findall("name")):
            self._SpewLocalizedNameNode(name_node)
        for gateway_node in apn_node.findall("gateway"):
            self._SpewString("gateway", gateway_node.text)
        for username_node in apn_node.findall("username"):
            self._SpewString("username", username_node.text)
        for password_node in apn_node.findall("password"):
            self._SpewString("password", password_node.text)
        for dns_node in sorted(apn_node.findall("dns")):
            self._SpewString("dns", dns_node.text)
        self._SpewMessageEnd("mobile_apn")

    def _SpewLocalizedNameNode(self, name_node):
        self._SpewMessageBegin("localized_name")
        self._SpewString("name", name_node.text)
        lang = name_node.get("xml:lang")
        if lang is not None:
            self._SpewString("language", lang)
        self._SpewMessageEnd("localized_name")

    def _SpewMessageBegin(self, message_name):
        self._SpewLine(message_name, "{")
        self._indent += 1
    ...
    def _PPrintXML(self, node):
        ...
        xml_data_str = reparsed.toprettyxml(indent="  ", encoding=FILE_ENCODING)
