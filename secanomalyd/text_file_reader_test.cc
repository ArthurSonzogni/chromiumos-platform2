...
TEST(TextFileReaderTest, InvalidFileTest) {
...
TEST(TextFileReaderTest, OpenFileTest) {
...
TEST(TextFileReaderTest, SeekToEndTest) {
  auto r = InitializeFileReaderForTest("TEST_MESSAGE_LOG");
  r->SeekToEnd();
  FileReaderRun want{};
  ReaderTest(r, want);
...
TEST(TextFileReaderTest, FileTextReaderTest) {
  auto r = InitializeFileReaderForTest("TEST_MESSAGE_LOG");
  std::string l1 =
...
// files.
class TextFileReaderConcurrentTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(TextFileReaderConcurrentTest, ReadAppendedTextTest) {
...
TEST_F(TextFileReaderConcurrentTest, ReadAfterSeekToEndTest) {
  std::vector<std::string> file_content{"line 1", "line 2", "end"};
...
TEST_F(TextFileReaderConcurrentTest, ReadDuringWriteTest) {
  auto r = std::make_unique<TextFileReader>(path_);
...
TEST_F(TextFileReaderConcurrentTest, ReadLineLongerThanBufferTest) {
  auto r = std::make_unique<TextFileReader>(path_);
...
// initialised.
TEST_F(TextFileReaderConcurrentTest, OpenFileRetryTest) {
...
TEST_F(TextFileReaderConcurrentTest, OpenFileRetryExceededTest) {
  // File <path_>.old does not exist yet.
  auto r = std::make_unique<TextFileReader>(path_.AddExtension("old"));
...
// file to the end.
TEST_F(TextFileReaderConcurrentTest, HandleFileMoveTest) {
