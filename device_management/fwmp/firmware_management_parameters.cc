...
struct fwmp::FirmwareManagementParametersRawV1_0 {
  uint8_t crc;
  uint8_t struct_size;
...
static_assert(sizeof(fwmp::FirmwareManagementParametersRawV1_0) == 40,
              "Unexpected size of FWMP");

// Index must match firmware; see README.firmware_management_parameters
const uint32_t fwmp::FirmwareManagementParameters::kNvramIndex = 0x100a;
const uint32_t fwmp::FirmwareManagementParameters::kNvramBytes =
    sizeof(struct FirmwareManagementParametersRawV1_0);
const uint32_t fwmp::FirmwareManagementParameters::kCrcDataOffset = 2;

// static
std::unique_ptr<fwmp::FirmwareManagementParameters>
fwmp::FirmwareManagementParameters::CreateInstance(
...
fwmp::FirmwareManagementParameters::FirmwareManagementParameters(
...
bool fwmp::FirmwareManagementParameters::Destroy(void) {
...
bool fwmp::FirmwareManagementParameters::Load(void) {
  if (loaded_) {
    return true;
  }
...
bool fwmp::FirmwareManagementParameters::Store(
...
bool fwmp::FirmwareManagementParameters::GetFlags(uint32_t* flags) {
  CHECK(flags);
...
bool fwmp::FirmwareManagementParameters::GetDeveloperKeyHash(
    brillo::Blob* hash) {
  CHECK(hash);
