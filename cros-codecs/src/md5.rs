...
const shift_amounts: [u8; 64] = [7, 12, 17, 22,  7, 12, 17, 22,
...
fn md5_digest_padded(padded_input: &[u8], mut A: u32, mut B: u32, mut C: u32, mut D: u32) -> String {
...
impl MD5Context {
    ...
    pub fn consume(&mut self, input: &[u8]) -> () {
        ...
        while self.buffer.len() >= 64 {
            (self.A, self.B, self.C, self.D) = process_chunk(&self.buffer[0..64], self.A, self.B, self.C, self.D);
...
mod tests {
    ...
    fn digest_simple_strings() {
        ...
        assert_eq!(md5_digest("The quick brown fox jumps over the lazy dog".as_bytes()),
