...
bool OwnershipIdTpm1::InitializeTpmManager() {
...
std::optional<std::string> OwnershipIdTpm1::Get() {
  ...
  if (status_reply.local_data().owner_delegate().secret().empty()) {
    LOG(WARNING) << "Empty owner delegate secret.";
    return kNoOwnerDelegate;
  }

  const std::string id = crypto::SHA256HashString(
      status_reply.local_data().owner_delegate().secret());
