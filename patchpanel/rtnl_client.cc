...
std::map<net_base::IPv6Address, MacAddress> RTNLClient::GetNeighborMacTable()
    const {
  sockaddr_nl kernel;
  ...
  bool done = false;
  std::map<net_base::IPv6Address, MacAddress> ret;
  while (!done) {
    ssize_t len = recvmsg(rtnl_fd_.get(), &rtnl_reply, 0);
    if (len < 0) {
    ...
    for (nlmsghdr* msg_ptr = reinterpret_cast<nlmsghdr*>(reply_buffer);
      ...
      switch (msg_ptr->nlmsg_type) {
        case NLMSG_DONE:
          ...
          break;
        case RTM_NEWNEIGH: {
          std::optional<net_base::IPv6Address> addr;
          ...
          for (; RTA_OK(rt_attr, rt_attr_len);
            ...
            if (rt_attr->rta_type == NDA_DST) {
              addr = net_base::IPv6Address::CreateFromBytes(
