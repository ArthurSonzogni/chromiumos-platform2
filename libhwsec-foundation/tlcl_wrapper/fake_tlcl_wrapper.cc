...
uint32_t FakeTlclWrapper::Close() {
  return TPM_SUCCESS;
}

void FakeTlclWrapper::Clear() {
  owner_auth_.clear();
  pcr_values_.clear();
#if !USE_TPM2
...
void FakeTlclWrapper::SetPCRValue(uint32_t index,
                                  const uint8_t value[TPM_PCR_DIGEST]) {
  memcpy(pcr_values_[index], value, TPM_PCR_DIGEST);
}

uint32_t FakeTlclWrapper::Extend(int pcr_num,
                                 const brillo::Blob& in_digest,
                                 brillo::Blob* out_digest) {
  // Not supported by the stub Tlcl.
  return TPM_E_INTERNAL_ERROR;
}

uint32_t FakeTlclWrapper::GetOwnership(bool* owned) {
  *owned = IsOwned();
  return TPM_SUCCESS;
}

uint32_t FakeTlclWrapper::GetRandom(uint8_t* data,
                                    uint32_t length,
                                    uint32_t* size) {
...
uint32_t FakeTlclWrapper::DefineSpace(uint32_t index,
                                      uint32_t perm,
                                      uint32_t size) {
  return DefineSpaceEx(nullptr, 0, index, perm, size, nullptr, 0);
}

uint32_t FakeTlclWrapper::DefineSpaceEx(const uint8_t* owner_auth,
  ...
  bool authenticated = !IsOwned() && nvram_spaces_.count(index) == 0;
#else
  brillo::SecureBlob in_auth(owner_auth, owner_auth + owner_auth_size);
  bool authenticated = IsOwned() && in_auth == owner_auth_;
  if (IsOwned() && (in_auth != owner_auth_)) {
...
uint32_t FakeTlclWrapper::GetPermissions(uint32_t index,
...
uint32_t FakeTlclWrapper::GetSpaceInfo(uint32_t index,
                                       uint32_t* permissions,
                                       uint32_t* size,
                                       void* auth_policy,
                                       uint32_t* auth_policy_size) {
...
uint32_t FakeTlclWrapper::Write(uint32_t index,
                                const void* data,
                                uint32_t length) {
  return WithSpace(index, [=](NvramSpaceData* space) {
...
uint32_t FakeTlclWrapper::Read(uint32_t index, void* data, uint32_t length) {
  return WithSpace(index, [=](NvramSpaceData* space) {
#if USE_TPM2
...
uint32_t FakeTlclWrapper::WriteLock(uint32_t index) {
  return WithSpace(index, [=](NvramSpaceData* space) {
    if (space->write_locked) {
      return TPM_E_INTERNAL_ERROR;  // should be TPM_AREA_LOCKED
    }
...
uint32_t FakeTlclWrapper::ReadLock(uint32_t index) {
  return WithSpace(index, [=](NvramSpaceData* space) {
    if (space->read_locked) {
      return TPM_E_INTERNAL_ERROR;  // should be TPM_AREA_LOCKED
    }
...
uint32_t FakeTlclWrapper::PCRRead(uint32_t index, void* data, uint32_t length) {
...
uint32_t FakeTlclWrapper::InitNvAuthPolicy(
  ...
  *auth_policy_size = SHA256_DIGEST_LENGTH;
  ...
  brillo::SecureBlob digest = Sha256ToSecureBlob(input);
  memcpy(auth_policy, digest.data(), digest.size());
  return TPM_SUCCESS;
}

uint32_t FakeTlclWrapper::GetVersion(uint32_t* vendor,
...
uint32_t FakeTlclWrapper::IFXFieldUpgradeInfo(TPM_IFX_FIELDUPGRADEINFO* info) {
...
uint32_t FakeTlclWrapper::ReadPubek(uint32_t* public_exponent,
                                    uint8_t* modulus,
...
uint32_t FakeTlclWrapper::TakeOwnership(
...
uint32_t FakeTlclWrapper::CreateDelegationFamily(uint8_t family_label) {
  if (IsOwned()) {
    return TPM_E_OWNER_SET;
  }
...
uint32_t FakeTlclWrapper::ReadDelegationFamilyTable(
...
uint32_t FakeTlclWrapper::WithSpace(uint32_t index, Action action) {
