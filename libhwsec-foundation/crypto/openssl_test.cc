...
std::string HexDecode(const std::string& hex) {
  std::string output;
  CHECK(base::HexStringToString(hex, &output));
  return output;
}
...
class OpenSSLTest : public testing::Test {
 public:
  ...
  void SetupPreGeneratedRsaKey() {
    std::string der_key = HexDecode(kRsaDerPrivateKey);
    const unsigned char* buf =
        reinterpret_cast<const unsigned char*>(der_key.data());
    rsa_key_.reset(d2i_RSAPrivateKey(nullptr, &buf, der_key.size()));
  }

  void SetupPreGeneratedEcKey() {
    std::string der_key = HexDecode(kEcDerPrivateKey);
    const unsigned char* buf =
        reinterpret_cast<const unsigned char*>(der_key.data());
...
TEST_F(OpenSSLTest, PreGeneratedKeyIsValid) {
  EXPECT_TRUE(rsa_key_);
  EXPECT_TRUE(ecc_key_);
}

TEST_F(OpenSSLTest, RSASubjectPublicKeyInfoToString) {
  std::string public_key = RSASubjectPublicKeyInfoToString(rsa_key_);
  EXPECT_EQ(public_key, HexDecode(kRsaDerPublicKey));
}

TEST_F(OpenSSLTest, ECCSubjectPublicKeyInfoToString) {
  std::string public_key = ECCSubjectPublicKeyInfoToString(ecc_key_);
  EXPECT_EQ(public_key, HexDecode(kEcDerPublicKey));
}
