...
plt.plot(num_elems, [sys.getsizeof(np.array(range(n)))
         for n in num_elems], '--')
legends.append('numpy.array(dtype=int)')
plt.plot(num_elems, [sys.getsizeof(np.array(range(n), dtype=int))
         for n in num_elems], '-.')
legends.append('numpy.array(dtype=numpy.float64)')
plt.plot(num_elems, [sys.getsizeof(
    np.array(range(n), dtype=np.float64)) for n in num_elems], ',')
legends.append('numpy.array(dtype=numpy.float32)')
plt.plot(num_elems, [sys.getsizeof(
    np.array(range(n), dtype=np.float32)) for n in num_elems])
legends.append('numpy.array(dtype=complex)')
plt.plot(num_elems, [sys.getsizeof(
    np.array(range(n), dtype=complex)) for n in num_elems])
legends.append('numpy.array(dtype=bool)')
plt.plot(num_elems, [sys.getsizeof(np.array(range(n), dtype=bool))
         for n in num_elems], 'x')
legends.append('numpy.array(dtype=numpy.uint8)')
plt.plot(num_elems, [sys.getsizeof(
    np.array(range(n), dtype=np.uint8)) for n in num_elems])
plt.xscale('log')
plt.yscale('log')
