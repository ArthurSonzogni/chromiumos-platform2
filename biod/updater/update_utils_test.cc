...
TEST_F(CrosFpUpdaterFindFirmwareTest, InvalidPathBlank) {
  ...
  auto status =
      FindFirmwareFile(base::FilePath(""), kValidBoardName, &out_file_path);
...
TEST_F(CrosFpUpdaterFindFirmwareTest, DirectoryWithoutFirmware) {
  ...
  auto status =
      FindFirmwareFile(GetTestTempDir(), kValidBoardName, &out_file_path);
...
TEST_F(CrosFpUpdaterFindFirmwareTest, OneGoodFirmwareFilePattern) {
  for (const auto& good_fw : kValidFirmwareNames) {
    ...
    auto status =
        FindFirmwareFile(GetTestTempDir(), good_fw.first, &out_file_path);
    // succeeds
