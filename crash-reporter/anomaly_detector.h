...
class KernelParser : public Parser {
 ...
 private:
  ...
  enum class KfenceOrKasanLineType {
    // The following enum values are used to parse the KFENCE or KASAN error.
    // The None value means that there is no error detected in the log. The
    // KfenceStart value means that the first line of the KFENCE dump was found.
    // KasanStart value means that the first line of the KASAN dump was found.
    None,
    KfenceStart,
    KasanStart,
  };
  ...
  KfenceOrKasanLineType kfence_or_kasan_last_line_ =
      KfenceOrKasanLineType::None;
  std::string kfence_or_kasan_text_;
  std::string text_;
  std::string flag_;
...
class DlcServiceParser : public Parser {
 public:
  explicit DlcServiceParser(bool testonly_send_all);
  MaybeCrashReport ParseLogEntry(const std::string& line) override;
