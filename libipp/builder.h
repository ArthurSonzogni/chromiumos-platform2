...
size_t IPP_EXPORT CalculateLengthOfBinaryFrame(const Frame& frame);
...
size_t IPP_EXPORT BuildBinaryFrame(const Frame& frame,
                                   uint8_t* buffer,
                                   size_t buffer_length);

// Return the binary representation of the frame as a vector.
std::vector<uint8_t> IPP_EXPORT BuildBinaryFrame(const Frame& frame);
