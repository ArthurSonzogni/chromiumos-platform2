...
constexpr base::TimeDelta kTransitionInterval = base::Milliseconds(10);

}  // namespace

class InternalBacklightTest : public TestEnvironment {
 public:
  InternalBacklightTest() : backlight_(kTransitionInterval) {}
...
TEST_F(InternalBacklightTest, BasicTest) {
  ...
  ASSERT_TRUE(backlight_.Init(this_test_path, "*"));
...
TEST_F(InternalBacklightTest, GranularityTest) {
  ...
  ASSERT_TRUE(backlight_.Init(this_test_path, "*"));
  EXPECT_EQ(20, backlight_.GetCurrentBrightnessLevel());
  EXPECT_EQ(255, backlight_.GetMaxBrightnessLevel());
}
...
TEST_F(InternalBacklightTest, NoDotDirsTest) {
  ...
  EXPECT_FALSE(backlight_.Init(this_test_path, "*"));
}
...
TEST_F(InternalBacklightTest, GlobTest) {
  ...
  ASSERT_TRUE(backlight_.Init(this_test_path, "*:kbd_backlight"));

  EXPECT_EQ(1, backlight_.GetCurrentBrightnessLevel());
  EXPECT_EQ(2, backlight_.GetMaxBrightnessLevel());
}
...
TEST_F(InternalBacklightTest, Transitions) {
  ...
  backlight_.clock()->set_current_time_for_testing(kStartTime);
  ASSERT_TRUE(backlight_.Init(test_path_, "*"));

  // An instant transition to the maximum level shouldn't use a timer.
  backlight_.SetBrightnessLevel(kMaxBrightness, base::TimeDelta());
  EXPECT_FALSE(backlight_.transition_timer_is_running());
  EXPECT_EQ(kMaxBrightness, ReadBrightness(backlight_dir));
  EXPECT_EQ(kMaxBrightness, backlight_.GetCurrentBrightnessLevel());
  ...
  backlight_.SetBrightnessLevel(kHalfBrightness, kDuration);

  // If the timeout fires at this point, we should still be at the maximum
  // level.
  EXPECT_TRUE(backlight_.transition_timer_is_running());
  EXPECT_EQ(kStartTime, backlight_.transition_timer_start_time());
  EXPECT_EQ(kTransitionInterval, backlight_.transition_timer_interval());
  EXPECT_TRUE(backlight_.TriggerTransitionTimeoutForTesting());
  EXPECT_EQ(kMaxBrightness, ReadBrightness(backlight_dir));
  EXPECT_EQ(kMaxBrightness, backlight_.GetCurrentBrightnessLevel());

  // Let half of the transition duration pass.
  const base::TimeTicks kMidpointTime = kStartTime + kDuration / 2;
  backlight_.clock()->set_current_time_for_testing(kMidpointTime);
  EXPECT_TRUE(backlight_.TriggerTransitionTimeoutForTesting());
  ...
  EXPECT_EQ(kMidpointBrightness, backlight_.GetCurrentBrightnessLevel());
  ...
  backlight_.clock()->set_current_time_for_testing(kEndTime);
  EXPECT_FALSE(backlight_.TriggerTransitionTimeoutForTesting());
  EXPECT_FALSE(backlight_.transition_timer_is_running());
  EXPECT_EQ(kHalfBrightness, ReadBrightness(backlight_dir));
  EXPECT_EQ(kHalfBrightness, backlight_.GetCurrentBrightnessLevel());
}
...
TEST_F(InternalBacklightTest, InterruptTransition) {
  ...
  backlight_.clock()->set_current_time_for_testing(base::TimeTicks() +
                                                   base::Microseconds(10000));
  ASSERT_TRUE(backlight_.Init(test_path_, "*"));

  // Start a one-second transition to 0.
  const base::TimeDelta kDuration = base::Seconds(1);
  backlight_.SetBrightnessLevel(0, kDuration);

  // Let the timer fire after half a second. The backlight should be at half
  // brightness.
  backlight_.clock()->set_current_time_for_testing(
      backlight_.clock()->GetCurrentTime() + kDuration / 2);
  EXPECT_TRUE(backlight_.TriggerTransitionTimeoutForTesting());
  ...
  const base::TimeTicks kInterruptedTransitionStartTime =
      backlight_.clock()->GetCurrentTime();
  backlight_.SetBrightnessLevel(kMaxBrightness, kDuration);
  EXPECT_TRUE(backlight_.transition_timer_is_running());
...
TEST_F(InternalBacklightTest, BlPower) {
  ...
  // written to
  // bl_power.
  backlight_.SetBrightnessLevel(1, base::TimeDelta());
  ...
  backlight_.clock()->set_current_time_for_testing(
      backlight_.clock()->GetCurrentTime() + kDuration / 2);
  ASSERT_TRUE(backlight_.TriggerTransitionTimeoutForTesting());
  ASSERT_NE(0, ReadBrightness(kDir));
  EXPECT_EQ(FB_BLANK_UNBLANK, ReadFile(kPowerFile));

  backlight_.clock()->set_current_time_for_testing(
      backlight_.clock()->GetCurrentTime() + kDuration / 2);
  ASSERT_FALSE(backlight_.TriggerTransitionTimeoutForTesting());
