...
TEST_F(BackendKeyManagementTpm1Test, GetEndorsementPublicKey) {
  for (auto encoding : {KeyManagement::PublicKeyEncoding::kRaw,
    ...
    brillo::Blob fake_public_key;
    brillo::Blob fake_modulus = brillo::BlobFromString("fake_modulus");
    brillo::Blob fake_key_blob = kFakeKeyBlob;
    if (encoding == KeyManagement::PublicKeyEncoding::kRaw) {
      EXPECT_CALL(proxy_->GetMockOveralls(),
                  Ospi_GetAttribData(kFakeEkHandle, TSS_TSPATTRIB_RSAKEY_INFO,
                                     TSS_TSPATTRIB_KEYINFO_RSA_MODULUS, _, _))
          .WillOnce(DoAll(SetArgPointee<3>(fake_modulus.size()),
                          SetArgPointee<4>(fake_modulus.data()),
                          Return(TPM_SUCCESS)));
      fake_public_key = fake_modulus;
    } else if (encoding == KeyManagement::PublicKeyEncoding::kDer) {
      EXPECT_CALL(proxy_->GetMockOveralls(),
          ...
          .WillOnce(DoAll(SetArgPointee<3>(fake_key_blob.size()),
                          ...
                          Return(TPM_SUCCESS)));
      fake_public_key = SetupGetPublicKeyDer(fake_key_blob);
    }

    EXPECT_THAT(backend_->GetKeyManagementTpm1().GetEndorsementPublicKey(
                    KeyAlgoType::kRsa, encoding),
                IsOkAndHolds(fake_public_key));
  }
}
