...
TEST(SignInHashTreeTest, GetAuxiliaryLabelsTest) {
...
TEST(SignInHashTreeTest, GenerateAndStoreHashCacheFile) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  ...
  ASSERT_TRUE(tree.GetLabelData(label, result_hash, cred_data, metadata_lost));
...
TEST(SignInHashTreeTest, InsertAndRetrieveLeafLabel) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  ...
  ASSERT_TRUE(tree->GetLabelData(SignInHashTree::Label(21, 6, 2), returned_hash,
                                 cred_data, metadata_lost));
...
TEST(SignInHashTreeTest, UpdateHashCacheOnInsertRemove) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
