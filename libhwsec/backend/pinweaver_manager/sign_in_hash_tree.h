...
#ifndef LIBHWSEC_BACKEND_PINWEAVER_MANAGER_SIGN_IN_HASH_TREE_H_
#define LIBHWSEC_BACKEND_PINWEAVER_MANAGER_SIGN_IN_HASH_TREE_H_
...
//
// |   HMAC(credential metadata)   |             credential metadata          |
// |                                |                                          |
//
...
class SignInHashTree {
 public:
  ...
  // inner hashes and updates the |inner_hash_array_|. Since we're using mmap
  // leaf cache in practice, this function should be called only when
  // regenerating the hash tree or LeafCache is somehow corrupted.
 ...
 private:
  ...
  uint8_t* GetInnerHashArrayPointer(uint32_t index);
  uint8_t* GetLeafCacheArrayPointer(uint32_t index);
...
#endif  // LIBHWSEC_BACKEND_PINWEAVER_MANAGER_SIGN_IN_HASH_TREE_H_
