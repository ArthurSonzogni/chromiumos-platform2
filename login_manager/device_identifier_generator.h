...
class DeviceIdentifierGenerator {
 public:
  ...
  using StateKeyCallback = base::OnceCallback<void(
      const base::expected<StateKeysData, StateKeysComputationError>&)>;
  ...
  // Computes the time quantum index for a given timestamp in seconds.
  static int64_t ComputeTimeQuantumIndex(int64_t time_seconds);
 ...
 private:
  // Returns computed state keys and current time quantum index or an error.
  //
  // Identifiers are generated by running a number of stable hardware
  ...
  base::expected<StateKeysData, StateKeysComputationError> ComputeKeys();
