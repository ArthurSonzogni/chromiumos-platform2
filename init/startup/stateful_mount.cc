      ...
      if (pv && pv->IsValid()) {
        ...
        if (vg && vg->IsValid()) {
          ...
          if (HibernateResumeBoot()) {
            ...
            lg->Activate();
            state_dev_ = base::FilePath(base::StringPrintf(
                "/dev/%s/%s", vg->GetName().c_str(), "unencrypted"));
            dev_image_ = base::FilePath(base::StringPrintf(
                "/dev/%s/%s", vg->GetName().c_str(), "dev-image"));
          }
        }
      }
