...
const char* OperandExtraParams::descriptor =
    "android.hardware.neuralnetworks.OperandExtraParams";

binder_status_t OperandExtraParams::readFromParcel(const AParcel* _parcel) {
  binder_status_t _aidl_ret_status;
  int32_t _aidl_tag;
  if ((_aidl_ret_status = ::ndk::AParcel_readData(_parcel, &_aidl_tag)) !=
      STATUS_OK) {
  ...
  switch (static_cast<Tag>(_aidl_tag)) {
    case channelQuant: {
      ::aidl::android::hardware::neuralnetworks::SymmPerChannelQuantParams
          _aidl_value;
      if ((_aidl_ret_status = ::ndk::AParcel_readData(_parcel, &_aidl_value)) !=
          STATUS_OK) {
        return _aidl_ret_status;
      }
...
binder_status_t OperandExtraParams::writeToParcel(AParcel* _parcel) const {
  binder_status_t _aidl_ret_status =
      ::ndk::AParcel_writeData(_parcel, static_cast<int32_t>(getTag()));
  if (_aidl_ret_status != STATUS_OK) {
    return _aidl_ret_status;
  }
  switch (getTag()) {
    case channelQuant:
      return ::ndk::AParcel_writeData(_parcel, get<channelQuant>());
    case extension:
      return ::ndk::AParcel_writeData(_parcel, get<extension>());
  }
