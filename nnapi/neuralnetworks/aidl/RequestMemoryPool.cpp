...
const char* RequestMemoryPool::descriptor =
    "android.hardware.neuralnetworks.RequestMemoryPool";
...
binder_status_t RequestMemoryPool::writeToParcel(AParcel* _parcel) const {
  ...
  switch (getTag()) {
    case pool:
      return ::ndk::AParcel_writeData(_parcel, get<pool>());
    case token:
      return ::ndk::AParcel_writeData(_parcel, get<token>());
  }
