...
class OperandExtraParams {
 public:
  ...
  OperandExtraParams()
      : _value(std::in_place_index<static_cast<size_t>(channelQuant)>,
               ::aidl::android::hardware::neuralnetworks::
                   SymmPerChannelQuantParams()) {}

  template <typename _Tp, typename = std::enable_if_t<_not_self<_Tp>>>
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr OperandExtraParams(_Tp&& _arg) : _value(std::forward<_Tp>(_arg)) {}

  template <size_t _Np, typename... _Tp>
  constexpr explicit OperandExtraParams(std::in_place_index_t<_Np>,
                                        _Tp&&... _args)
      : _value(std::in_place_index<_Np>, std::forward<_Tp>(_args)...) {}

  template <Tag _tag, typename... _Tp>
  static OperandExtraParams make(_Tp&&... _args) {
    return OperandExtraParams(std::in_place_index<static_cast<size_t>(_tag)>,
                              std::forward<_Tp>(_args)...);
  }

  template <Tag _tag, typename _Tp, typename... _Up>
  static OperandExtraParams make(std::initializer_list<_Tp> _il,
                                 _Up&&... _args) {
    return OperandExtraParams(std::in_place_index<static_cast<size_t>(_tag)>,
  ...
  inline std::string toString() const {
    ...
    switch (getTag()) {
      case channelQuant:
        os << "channelQuant: "
           << ::android::internal::ToString(get<channelQuant>());
        break;
      case extension:
        os << "extension: " << ::android::internal::ToString(get<extension>());
        break;
    }
    os << "}";
    return os.str();
 ...
 private:
  std::variant<
      ::aidl::android::hardware::neuralnetworks::SymmPerChannelQuantParams,
      std::vector<uint8_t>>
      _value;
};
}  // namespace neuralnetworks
...
constexpr inline std::array<
    aidl::android::hardware::neuralnetworks::OperandExtraParams::Tag,
    2>
    enum_values<
        aidl::android::hardware::neuralnetworks::OperandExtraParams::Tag> = {
        aidl::android::hardware::neuralnetworks::OperandExtraParams::Tag::
