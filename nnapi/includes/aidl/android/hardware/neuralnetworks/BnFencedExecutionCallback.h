...
class BnFencedExecutionCallback
    : public ::ndk::BnCInterface<IFencedExecutionCallback> {
 public:
...
class IFencedExecutionCallbackDelegator : public BnFencedExecutionCallback {
 public:
  explicit IFencedExecutionCallbackDelegator(
  ...
  ::ndk::ScopedAStatus getExecutionInfo(
      ...
      override {
    return _impl->getExecutionInfo(out_timingLaunched, out_timingFenced,
                                   _aidl_return);
  }
