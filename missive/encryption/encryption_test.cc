...
class EncryptionTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_OK(encryptor_result) << encryptor_result.error();
  ...
  StatusOr<std::string> DecryptMatchingSecret(
      Encryptor::PublicKeyId public_key_id, std::string_view encrypted_key) {
...
TEST_F(EncryptionTest, EncryptAndDecrypt) {
  ...
  ASSERT_OK(decrypt_secret_result) << decrypt_secret_result.error();
  ...
  ASSERT_OK(decrypted_result) << decrypted_result.error();

  EXPECT_THAT(decrypted_result.value(), ::testing::StrEq(kTestString));
}
...
TEST_F(EncryptionTest, EncryptAndDecryptMultiple) {
  ...
  for (size_t i = 0; i < encrypted_records.size(); ++i) {
    ...
    // Verify match.
    EXPECT_THAT(decrypted_result.value(), ::testing::StrEq(kTestStrings[i]));
  }
}
...
TEST_F(EncryptionTest, EncryptAndDecryptMultipleParallel) {
      ...
      encryptor_->OpenRecord(base::BindOnce(
          [](SingleEncryptionContext* self,
            ...
            if (!handle_result.has_value()) {
              self->Respond(base::unexpected(std::move(handle_result).error()));
              return;
            }
            base::ThreadPool::PostTask(
                FROM_HERE,
    ...
    void CloseRecord(Encryptor::Handle* handle) {
      handle->CloseRecord(base::BindOnce(
          [](SingleEncryptionContext* self,
            ...
            self->Respond(std::move(encryption_result));
          },
          base::Unretained(this)));
    }
   ...
   private:
    ...
    void DecryptSharedSecret(std::string_view private_key) {
      ...
      if (!shared_secret_result.has_value()) {
        Respond(std::move(shared_secret_result));
        return;
      }
      base::ThreadPool::PostTask(
          FROM_HERE,
  ...
  for (size_t i = 0; i < decryption_results.size(); ++i) {
    ...
    ASSERT_OK(decryption_result) << decryption_result.error();
    // Verify data match.
    EXPECT_THAT(decryption_result.value(), ::testing::StrEq(kTestStrings[i]));
  }
}
