...
RecoveryCryptoFakeTpmBackendImpl::GenerateDiffieHellmanSharedSecret(
  ...
  crypto::ScopedBIGNUM own_priv_key_bn =
      SecureBlobToBigNum(request.encrypted_own_priv_key);
  if (!own_priv_key_bn) {
    LOG(ERROR) << "Failed to convert SecureBlob to BIGNUM";
    return nullptr;
  }
  crypto::ScopedEC_POINT point_dh = request.ec.Multiply(
      *request.others_pub_point, *own_priv_key_bn, context.get());
  if (!point_dh) {
