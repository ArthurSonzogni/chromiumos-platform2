...
class AuthFactorDriver {
 public:
  ...
  virtual void PrepareForAdd(const PrepareInput& prepare_input,
  ...
  // Given an AuthFactor instance, attempt to determine how much time there is
  // until this factor expires. Returns a not-OK status if the expiration cannot
  // be determined or the type does not support expiration.
  virtual CryptohomeStatusOr<base::TimeDelta> GetTimeUntilExpiration(
      const ObfuscatedUsername& username, const AuthFactor& factor) const = 0;
