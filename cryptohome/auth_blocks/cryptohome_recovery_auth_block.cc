...
std::unique_ptr<AuthBlock> CryptohomeRecoveryAuthBlock::New(
  ...
  return std::make_unique<CryptohomeRecoveryAuthBlock>(
      &hwsec, &recovery_hwsec, &hwsec_pw_manager, &platform);
}
...
void CryptohomeRecoveryAuthBlock::Create(
  ...
  if (revocation::IsRevocationSupported(hwsec_)) {
    ...
    CryptoStatus result = revocation::Create(
        hwsec_pw_manager_, &revocation_state, key_blobs.get());
    if (!result.ok()) {
      std::move(callback).Run(
          MakeStatus<CryptohomeCryptoError>(
              CRYPTOHOME_ERR_LOC(
...
void CryptohomeRecoveryAuthBlock::Derive(const AuthInput& auth_input,
  ...
  brillo::Blob serialized_epoch_response =
  ...
  const brillo::Blob& ephemeral_pub_key =
  ...
  brillo::Blob serialized_response_proto =
  ...
  if (!epoch_response.ParseFromArray(serialized_epoch_response.data(),
                                     serialized_epoch_response.size())) {
  ...
  if (!response_proto.ParseFromArray(serialized_response_proto.data(),
                                     serialized_response_proto.size())) {
  ...
  if (state.revocation_state.has_value()) {
    ...
    CryptoStatus result = revocation::Derive(
        hwsec_pw_manager_, state.revocation_state.value(), key_blobs.get());
    if (!result.ok()) {
