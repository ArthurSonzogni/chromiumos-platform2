...
umask 022
mkdir -p "${LOCKBOX_CACHE_DIR}"
chmod 0711 "${LOCKBOX_CACHE_DIR}"
...
if [ -f "${OLD_INSTALL_ATTRS_FILE}" ]; then
  if [ ! -f "${NEW_INSTALL_ATTRS_FILE}" ]; then
    ...
    mkdir -p "${INSTALL_ATTRS_NEW_DIR}"
    cp "${OLD_INSTALL_ATTRS_FILE}" "${DUP_INSTALL_ATTRS_FILE}"
    mv "${DUP_INSTALL_ATTRS_FILE}" "${NEW_INSTALL_ATTRS_FILE}"
  fi
