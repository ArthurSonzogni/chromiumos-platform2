...
pub mod quota {
    // Required for keeping linter happy while using automock on the module.
    #![allow(dead_code)]
    ...
    // ![allow(dead_code)] is not being applied to the consts below, so we have
    ...
    pub fn set_quota_normal(path: &Path) -> Result<()> {
    ...
    pub fn set_quota_limited(path: &Path) -> Result<()> {
        let path_str = path.to_str().context("Failed to convert path to string")?;
