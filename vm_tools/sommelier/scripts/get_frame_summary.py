...
class FrameLog():
    ...
    def output_fps(self, surface, windows, max_ft_ms, target_fps):
        ...
        ft_target_ms = self.get_target_ft(target_fps,
                                          len(events) / total_sec)
        for w_sec in windows + [total_sec]:
            ...
            for event in events:
                ...
                if current_sec > w_sec - _FP_ERROR:
                    ...
                    print('-------------------------------')
                    print(
                        f'FPS: {frame_count / current_sec}')
                    print(
                        f'Max frame time: {max_frame_ms} ms')
    ...
    def output_fps_sliding(self, surface, window, max_ft_ms):
        ...
        for i in range(window, len(events)):
            ...
            frames_ms = [(sl_window[x].time - sl_window[x+1].time)
                         * _MS_PER_SEC for x in range(window-1)]
        ...
        if len(cvars) > 1:
            print('Arithmetic mean of coefficient of variation:',
                  statistics.fmean(cvars))
            print('Geometric mean of coefficient of variation:',
                  statistics.geometric_mean(cvars))
        if len(max_ft_events) > 1:
            print(f'Average number of frame time events over threshold'
                  f' {max_ft_ms} ms:', statistics.mean(max_ft_events))
        print()
    print()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-frame-time', type=float,
                        ...
                        default=200)
    parser.add_argument('--sliding', type=int,
