...
TEST(VmmSwapUsagePolicyTest, OnShutdownWriteEntriesToFile) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath history_file_path = temp_dir.GetPath().Append("usage_history");
  base::Time now = base::Time::Now();
  ...
  before_policy.OnEnabled(now - base::Days(7));
  before_policy.OnShutdown(now - base::Days(5));
...
TEST(VmmSwapUsagePolicyTest, OnShutdownWithoutDisable) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath history_file_path = temp_dir.GetPath().Append("usage_history");
  base::Time now = base::Time::Now();
  ...
  // 14 days + 7 days
  before_policy.OnEnabled(now - base::Days(14));
  before_policy.OnShutdown(now - base::Days(1));
...
TEST(VmmSwapUsagePolicyTest, OnShutdownLatestEnableAfter1hour) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath history_file_path = temp_dir.GetPath().Append("usage_history");
  base::Time now = base::Time::Now();
...
TEST(VmmSwapUsagePolicyTest, OnShutdownLatestEnableWithin1hour) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath history_file_path = temp_dir.GetPath().Append("usage_history");
  base::Time now = base::Time::Now();
  ...
  before_policy.OnEnabled(now - base::Days(7) - base::Minutes(10));
  // Write entry as if enabled 1 hour later than last enable.
  before_policy.OnShutdown(now - base::Days(7) - base::Minutes(5));
  ASSERT_EQ(before_policy.PredictDuration(now), base::Days(4));
...
TEST(VmmSwapUsagePolicyTest, InitMultipleShutdownRecordAreIgnored) {
  ...
  // 3 days
  before_policy.OnEnabled(now - base::Days(14));
  before_policy.OnShutdown(now - base::Days(12));
  before_policy.OnDisabled(now - base::Days(11));
  // 5 days
  before_policy.OnEnabled(now - base::Days(7));
  before_policy.OnShutdown(now - base::Days(6));
...
TEST(VmmSwapUsagePolicyTest, OnShutdownRotateHistoryFile) {
  base::ScopedTempDir temp_dir;
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath history_file_path = temp_dir.GetPath().Append("usage_history");
  base::Time now = base::Time::Now();
