...
GcamAeConfig::GcamAeConfig(const char* default_config_file_path,
  ...
  bool ret = override_file_path_watcher_.Watch(
      ...
      base::BindRepeating(&GcamAeConfig::OnConfigFileUpdated,
                          base::Unretained(this)));
  if (!ret) {
    LOGF(ERROR) << "Can't monitor Gcam AE config file path: "
...
GcamAeConfig::Options GcamAeConfig::GetOptions() {
  base::AutoLock l(options_lock_);
  return options_;
}

bool GcamAeConfig::ReadConfigFile(const base::FilePath& file_path) {
  ...
  if (!json_values) {
    LOGF(ERROR) << "Failed to load the content of Gcam AE config file";
    return false;
  }
...
void GcamAeConfig::OnConfigFileUpdated(const base::FilePath& file_path,
                                       bool error) {
  ReadConfigFile(override_config_file_path_);
}
