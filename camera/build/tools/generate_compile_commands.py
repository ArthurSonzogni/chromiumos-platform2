...
PORTAGE_TMPDIR = "/build/{board}/tmp/portage"
PORTAGE_CACHEDIR = "/build/{board}/var/cache/portage"
...
def get_canonical_package_name(board: str, pkg: str) -> str:
    ...
    ebuild_package_path = pathlib.Path(output.stdout.strip()).parent
    pkg_name = ebuild_package_path.name
    pkg_category = ebuild_package_path.parent.name
    return "/".join((pkg_category, pkg_name))


def get_portage_gendirs(board: str, pkg: str) -> List[pathlib.Path]:
    cmd = [f"equery-{board}", "which", pkg]
    try:
        output = subprocess.run(
            cmd, check=True, encoding="utf-8", stdout=subprocess.PIPE
        ...
        raise ValueError(f"Unknown package: {pkg}")
    pkg_ebuild_with_version = pathlib.Path(output.stdout.strip()).stem
    ebuild_package_path = pathlib.Path(output.stdout.strip()).parent
    pkg_name = ebuild_package_path.name
    pkg_category = ebuild_package_path.parent.name

    portage_tmp_gendir = pathlib.Path(
        PORTAGE_TMPDIR.format(board=board),
        pkg_category,
        pkg_ebuild_with_version,
        "work",
        "build",
        "out",
        "Default",
    ...
    portage_cache_gendir = pathlib.Path(
        PORTAGE_CACHEDIR.format(board=board),
        pkg_category,
        pkg_name,
        "out",
        "Default",
    ...
    logging.debug("tmp dirs: %s %s", portage_tmp_gendir, portage_cache_gendir)
    return [portage_tmp_gendir, portage_cache_gendir]
    ...
    for cmd in compdb:
        ...
        for k, v in patterns.items():
            cmd[KEY_COMMAND] = re.sub(
                k,
                "-I%s/%s" % (pathlib.Path(src_root, v), r"\1"),
                cmd[KEY_COMMAND],
            )
...
def emerge_packages(board: str, packages: List[str], use_flags: str):
    ...
    # FEATURES=noclean for preserving the gen outputs.
    emerge_env["FEATURES"] = "noclean"
    emerge_env["USE"] = " ".join([emerge_env["USE"], flags])
...
def aggregate_compile_db(
    ...
    for p in packages:
        ...
        if compdb_path.exists():
            compdb_files.append(compdb_path.as_posix())
...
def auto_cros_workon_packages(board: str, packages: List[str]):
    ...
    cros-workon state, or None if no package needs to be cros-workon'd.
    """
...
def main(argv: list):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output directory to store a copy of the generated files "
            "(default=%(default)s)"
        ),
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.gen_dir is not None:
            ...
            for p in all_packages:
                for d in get_portage_gendirs(args.board, p):
                    if d.exists():
                        logging.info("Copying generated files from %s", d)
                        shutil.copytree(d, args.gen_dir, dirs_exist_ok=True)

        compdb_aggregated = []
        if args.append and pathlib.Path(args.output_file).exists():
            logging.info(
