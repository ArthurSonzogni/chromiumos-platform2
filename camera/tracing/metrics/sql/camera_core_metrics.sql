...
DROP TABLE IF EXISTS result_functions;
CREATE TABLE result_functions (name varchar(80));
INSERT INTO result_functions VALUES
  -- ProcessCaptureResult
  ("Camera3CallbackOpsDelegate::ProcessCaptureResultOnThread"),
...
-- The configuration ID, partial result, number of output buffers of a process
-- capture result.
DROP VIEW IF EXISTS result_with_num_output_buffers;
CREATE VIEW result_with_num_output_buffers AS
SELECT DISTINCT
  conf_id,
  EXTRACT_ARG(arg_set_id, 'debug.partial_result') AS partial_result,
  EXTRACT_ARG(arg_set_id, 'debug.num_output_buffers') AS num_output_buffers
FROM slice_per_session_with_stream_conf_id
WHERE name = 'CameraDeviceAdapter::ProcessCaptureResult';


-- Create a view that calculate the minimum, average, standard deviation,
-- maximum and count of the duration for each configuration ID, function name
-- and partial result for functions in |result_functions|.
DROP VIEW IF EXISTS result_metrics;
...
WITH avg AS (
  ...
  FROM slice_per_session_with_stream_conf_id
  WHERE name IN (SELECT name FROM result_functions) AND conf_id NOT NULL
  GROUP BY conf_id, name, partial_result
)
  SELECT
    conf_id,
    EXTRACT_ARG(arg_set_id, 'debug.partial_result') AS partial_result,
...
UNION ALL
  SELECT
    conf_id, partial_result, name, metric_name, unit,
    CAST(value / 1e3 AS INT) AS value, bigger_is_better
  FROM avg
UNION ALL
  SELECT
    s.conf_id AS conf_id,
    EXTRACT_ARG(s.arg_set_id, 'debug.partial_result') AS partial_result,
  ...
  GROUP by s.conf_id, s.name, partial_result
UNION ALL
  SELECT
    conf_id,
    EXTRACT_ARG(arg_set_id, 'debug.partial_result') AS partial_result,
          ...
          SELECT RepeatedField(
            CameraStreamMetrics(
              ...
              'result_buffer_metrics', (
                ...
                WHERE distinct_result_buffer.conf_id = distinct_stream.conf_id
              ),
              'result_metrics', (
                SELECT RepeatedField(
                  CaptureResultMetrics(
                    'partial_result_number', distinct_result_metrics.partial_result,
                    'num_output_buffers', IFNULL(result_with_num_output_buffers.num_output_buffers, 0),
                    'function_metrics', (
                      SELECT RepeatedField(
                        FunctionMetrics(
                          'function_name', name,
                      ...
                      FROM result_metrics AS result
                      WHERE result.partial_result = distinct_result_metrics.partial_result
                    )
                  )
                )
                FROM (
                  SELECT DISTINCT conf_id, partial_result
                  FROM result_metrics
                ) AS distinct_result_metrics
                LEFT JOIN result_with_num_output_buffers USING (conf_id, partial_result)
                WHERE distinct_result_metrics.conf_id = distinct_stream.conf_id
              )
            )
          )
          FROM (
