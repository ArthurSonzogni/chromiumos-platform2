 ...
 public:
  ...
  DlcClientImpl(
      ...
      : dlc_root_path_cb_(std::move(dlc_root_path_cb)),
        ...
        weak_factory_(this) {
    LOGF(INFO) << "Setting up DlcClient";

    dbus::Bus::Options opts;
    opts.bus_type = dbus::Bus::SYSTEM;
    bus_ = new dbus::Bus(std::move(opts));
    if (!bus_->Connect()) {
      LOGF(ERROR) << "Failed to connect to system bus";
    }
    LOGF(INFO) << "Connected to system bus";
    ...
    LOGF(INFO) << "DlcClient setup complete";
  }
  ...
  void OnDlcStateChanged(const dlcservice::DlcState& dlc_state) {
    ...
    switch (dlc_state.state()) {
      case dlcservice::DlcState::INSTALLED:
        LOGF(INFO) << "Successfully installed DLC " << kDlcId << " at "
                   << dlc_state.root_path();
        InvokeSuccessCb(base::FilePath(dlc_state.root_path()));
        break;
      case dlcservice::DlcState::INSTALLING:
        LOGF(INFO) << static_cast<int>(dlc_state.progress() * 100)
  ...
  void InstallDlc() override {
    ...
    if (dlc_state.state() == dlcservice::DlcState::INSTALLED) {
      LOGF(INFO) << "dlc " << kDlcId << " already installed at "
                 << dlc_state.root_path();
      InvokeSuccessCb(base::FilePath(dlc_state.root_path()));
    } else {
      LOGF(INFO) << "dlc " << kDlcId
...
std::unique_ptr<DlcClient> DlcClient::Create(
    base::OnceCallback<void(const base::FilePath&)> dlc_root_path_cb,
    base::OnceCallback<void(const std::string&)> error_cb) {
  return std::make_unique<DlcClientImpl>(std::move(dlc_root_path_cb),
                                         std::move(error_cb));
}
