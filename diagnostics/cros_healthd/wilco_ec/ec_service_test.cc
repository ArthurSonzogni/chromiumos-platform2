...
TEST_P(WilcoEcEventTest, GetReason) {
...
class WilcoEcServiceTest : public testing::Test {
 protected:
  WilcoEcServiceTest() = default;
...
TEST_F(WilcoEcServiceTest, Start) {
  CreateEcEventFile();
  ASSERT_TRUE(service()->Start());
}

TEST_F(WilcoEcServiceTest, StartFailure) {
  ASSERT_FALSE(service()->Start());
}

// Tests for the EcService class that started successfully.
class StartedWilcoEcServiceTest : public WilcoEcServiceTest {
 protected:
  void SetUp() override {
    ASSERT_NO_FATAL_FAILURE(WilcoEcServiceTest::SetUp());
    CreateEcEventFile();
    ASSERT_TRUE(service()->Start());
    InitFifoWriteEnd();
  }
...
TEST_F(StartedWilcoEcServiceTest, ReadEvent) {
  base::RunLoop run_loop;
...
TEST_F(StartedWilcoEcServiceTest, ReadManyEvent) {
  base::RunLoop run_loop;
